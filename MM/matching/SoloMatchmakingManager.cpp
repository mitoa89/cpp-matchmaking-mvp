...
CSoloMatchmakingManager::CSoloMatchmakingManager() = default;

bool CSoloMatchmakingManager::Init() {
    m_IsRunning = true;
    ...
    m_MatchmakingRules.DoAll([&](std::shared_ptr<const MatchmakingRule> matchmaking_rule) {
        ...
        for (const auto worldGroup : MatchmakingConfig::kDefaultWorldGroups) {
            auto matchmaking_worker = std::make_shared<CSoloMatchmakingWorkerGroup>(
                MatchmakingGroupKey(worldGroup, matchmaking_rule->m_MatchZoneID),
                matchmaking_rule, MatchmakingConfig::kDefaultWorkerCount);

            m_MatchmakingWorkers.Insert(matchmaking_worker->GetWorkerGroupKey(), matchmaking_worker);
        }
    });

    m_MatchmakingWorkers.Start();

    return true;
}

void CSoloMatchmakingManager::Terminate() {
    m_IsRunning = false;

    m_MatchmakingWorkers.Stop();

}

bool CSoloMatchmakingManager::CleanupPrevMatchmaking(const MatchTicketRequest &match_ticket_request) {
...
bool CSoloMatchmakingManager::CleanupPrevMatchmaking(const PLAYER_KEY &player_key) {
    bool cleanup_processed = false;

    cleanup_processed |= (nullptr != StopMatchmaking(player_key, false));
    cleanup_processed |= GetIntegrationZoneInstanceManager()->Process_PlayerLeave_ToIntegrationServer(player_key, false);

    return cleanup_processed;
}

CMatchTicketPtr CSoloMatchmakingManager::StartMatchmaking(MatchTicketRequest &&match_ticket_request) {
    if (m_IsRunning == false) {
        return nullptr;
    }

    if (match_ticket_request.m_Players.size() > 1) {
        return nullptr;
    }
    ...
    if (false == EnqueueWorkerJobRequest(
            std::make_shared<CMatchmakingRequest>(match_ticket_ptr->GetMatchingGroupKey(), match_ticket_ptr))) {
...
CMatchTicketPtr CSoloMatchmakingManager::StopMatchmaking(const PLAYER_KEY &player_key, bool apply_dodge_penalty) {
    auto match_ticket_ptr = FindMatchTicketByPlayerKey(player_key);
    if (match_ticket_ptr == nullptr) {
        return nullptr;
    }

    // 수락 대기 중 취소 요청은 거절 응답 처리
    if (match_ticket_ptr->GetStatus() == EMatchTicketStatus::REQUIRES_ACCEPTANCE)
    {
        GetMatchSessionManager()->AcceptMatchSession(PlayerMatchAcceptInfo(player_key, false, false));
        return match_ticket_ptr;
    }

    if (false == nMatchTicketStatusEx::CanChangeStatus(match_ticket_ptr->GetStatus(),
                                                       EMatchTicketStatus::CANCELLED)) {
        return match_ticket_ptr;
    }

    if (auto worker_group_ptr = m_MatchmakingWorkers.GetWorkerGroup(match_ticket_ptr->GetMatchingGroupKey())) {
        if (false == worker_group_ptr->CancelMatchTicket(match_ticket_ptr)) {
            return match_ticket_ptr;
        }
    }
...
bool CSoloMatchmakingManager::EnqueueWorkerJobRequest(CMatchmakingRequestPtr matchmaking_request_ptr) {
    if (nullptr == matchmaking_request_ptr)
        return false;

    auto worker_group_ptr = m_MatchmakingWorkers.GetWorkerGroup(matchmaking_request_ptr->m_WorkerGroupKey);
...
void CSoloMatchmakingManager::OnFailedMatchmaking(CMatchTicketPtr match_ticket_ptr, nProtocol_Result &&status_reason,
                                              std::string &&status_message) {
    if (match_ticket_ptr == nullptr)
        return;

    if (false == match_ticket_ptr->ChangeStatus(EMatchTicketStatus::FAILED)) {
        std::cout << std::format("error failed to fail match ticket:{} on failed match session : {} {}",
                                 match_ticket_ptr->GetMatchTicketID(), status_reason.asInt(),
    ...
    std::cout << std::format("match failed ticket:{} on failed match session : {} {}",
                             match_ticket_ptr->GetMatchTicketID(), status_reason.asInt(), status_message) << std::endl;
...
void CSoloMatchmakingManager::OnExpireMatchmaking(const MATCH_TICKET_ID &match_ticket_id) {
    EraseMatchTicket(match_ticket_id);
}

bool CSoloMatchmakingManager::InsertMatchTicket(CMatchTicketPtr match_ticket_ptr) {
    if (match_ticket_ptr == nullptr)
        return false;

    {
        ...
        for (auto &&player_info: match_ticket_ptr->GetPlayers()) {
            if (false == m_MatchTicketFindMapByPlayerKey.emplace(player_info.m_PlayerKey,
                                                                 match_ticket_ptr->GetMatchTicketID()).second) {
...
CMatchTicketPtr CSoloMatchmakingManager::EraseMatchTicket(const MATCH_TICKET_ID &match_ticket_id) {
    std::unique_lock lock(m_MatchmakingMutex);
...
CMatchTicketPtr CSoloMatchmakingManager::FindMatchTicketByPlayerKey(const PLAYER_KEY &player_key) const {
...
CMatchTicketPtr CSoloMatchmakingManager::FindMatchTicket(const MATCH_TICKET_ID &match_ticket_id) const {
    std::shared_lock lock(m_MatchmakingMutex);
    auto it = m_MatchTicketMap.find(match_ticket_id);
    if (it == m_MatchTicketMap.end())
        return nullptr;
...
bool CSoloMatchmakingManager::InsertMatchmakingRule(MatchmakingRule &&matchmaking_rule) {
    return m_MatchmakingRules.Insert(matchmaking_rule.m_MatchZoneID,
                                     std::make_shared<MatchmakingRule>(std::move(matchmaking_rule)));
}

size_t CSoloMatchmakingManager::GetMatchmakingRequestSize() const {
    std::shared_lock lock(m_MatchmakingMutex);
